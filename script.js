...
function addCard(cardsArray, currentDOM, currDiv) {
  ...
  newCard.classList.add(
    ...
    'card',
    `${activePlayer === 0 ? 'dealer-card-1' : 'player--card'}`,
    `${
      activePlayer === 0
        ? 'dealer-card-1'
        : activePlayer === 1
...
function hit(currentScore, cardsArray, card1, card2, currentDOM, currDiv) {
  if (currentScore < 21) {
    const emptyCardIndex = cardsArray.findIndex(
      card => card.src.slice(28).split('_')[0] === '0'
    );
    ...
    if (emptyCardIndex !== -1) {
      ...
      emptyCardIndex === 0
        ? (cardsArray[emptyCardIndex].src = card1.src)
        : (cardsArray[emptyCardIndex].src = card2.src);
      currentDOM.textContent = calcScore(cardsArray);
    } else {
      addCard(cardsArray, currentDOM, currDiv);
    }
  }
}
// stick function
async function stick() {
  // 1 PLAYER GAME
  if (noOfPlayers === 1 && player01CurrentScore < 21) {
    dealersTurn();
  }
  ...
  if (noOfPlayers === 2 && activePlayer === 1) {
    ...
    player01Section.classList.remove('player--active');
    player02Section.classList.add('player--active');
  } else if (noOfPlayers === 2 && activePlayer === 2) {
    dealersTurn();
  }
}
...
async function dealersTurn() {
  ...
  if (player01CurrentScore <= 21 && player02CurrentScore <= 21) {
    highestScore = Math.max(player01CurrentScore, player02CurrentScore);
  } else if (player01CurrentScore <= 21 && player02CurrentScore > 21) {
    highestScore = +player01CurrentScore;
  } else if (player01CurrentScore > 21 && player02CurrentScore <= 21) {
    highestScore = +player02CurrentScore;
  }
  ...
  while (+dealerCurrentScore <= +highestScore) {
    ...
    if (noOfPlayers === 1) {
      if (+dealerCurrentScore > 21) {
        player01Wins();
      } else if (+dealerCurrentScore === 21 && +player01CurrentScore === 21) {
        player01Draw();
      } else if (+dealerCurrentScore > +player01CurrentScore) {
        dealerWins();
      }
      ...
      if (+dealerCurrentScore > 21) {
        if (player01CurrentScore <= 21 && player02CurrentScore <= 21) {
          player01And02Wins();
        } else if (player01CurrentScore <= 21) {
          player01Wins();
        } else if (player02CurrentScore <= 21) {
          player02Wins();
        }
      ...
      } else if (+dealerCurrentScore > highestScore) {
        dealerWins();
      }
    }
  }
...
function checkWinOnePlayer() {
  ...
  if (+player01CurrentScore > 21) {
    // acesInHandCountedAsEleven = 0;
    return dealerWins();
  }

  // player 01 gets 21, dealers turn
  if (+player01CurrentScore === 21)
    return dealersTurn(player01Section, player01CurrentScore);
}
...
hitBtn.addEventListener('click', async function () {
  ...
  // 2 PLAYER GAME
    ...
    // PLAYER 02 TURN
      ...
      if (+player02CurrentScore > 21 && +player01CurrentScore > 21) {
        dealerWins();
      }
      // PLAYER 02 BUST AND PLAYER 01 CURRENT SCORE <= 21 DEALERS TURN
      else if (+player02CurrentScore > 21 && +player01CurrentScore <= 21) {
        dealersTurn(player01Section, player01CurrentScore);
      }
      // PLAYER 02 GETS 21 DEALERS TURN
      else if (+player02CurrentScore === 21) {
        dealersTurn(player02Section, player02CurrentScore);
      }
    }
  }
});
